...
_.parseArticle = function(){
	...
	return $('#article', '.app_articleleft').html();
};
...
_.parseAuthor = function(){
    ...
    return $('#author','.title_author').find('li').text();
...
_.parseWrittenTime = function(){
    var that = this,
        $ = that.loaded;
    var written = $('.date_ctrl', '.app_articleleft').find($('#date_text')).text();
...
_.parseCategory = function(){
    var that = this,
        $ = that.loaded;
    return $('.art_title', '.app_articleleft').find('img').attr('alt');
};

_.parseImage = function(){
    var that = this,
        $ = that.loaded;
    return $('#article', '.app_articleleft').find('img').attr('src');
